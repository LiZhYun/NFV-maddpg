...
def prepro(hp):
    ...
    # train
    _prepro = lambda x:  [line.strip() for line in open(x, 'r', encoding='utf-8', errors='ignore').read().split("\n") \
                      if not line.startswith("<")]  # 去掉标签行
    ...
    _prepro = lambda x: [re.sub("<[^>]+>", "", line).strip() \
                         for line in open(x, 'r', encoding='utf-8', errors='ignore').read().split("\n")
    ...
    def _write(sents, fname):
        with open(fname, 'w', encoding='utf-8', errors='ignore') as fout:
            fout.write("\n".join(sents))
    ...
    train = '--input=iwslt2016/prepro/train --pad_id=0 --unk_id=1 \
             ...
             --model_type=bpe'.format(hp.vocab_size)             # 对于vnf 此处字典需要更改为vnf种类
    ...
    print("train1:", open("iwslt2016/segmented/train.de.bpe",
                          'r', encoding='utf-8', errors='ignore').readline())
    print("train2:", open("iwslt2016/segmented/train.en.bpe",
                          'r', encoding='utf-8', errors='ignore').readline())
    print("eval1:", open("iwslt2016/segmented/eval.de.bpe",
                         'r', encoding='utf-8', errors='ignore').readline())
    print("eval2:", open("iwslt2016/segmented/eval.en.bpe",
                         'r', encoding='utf-8', errors='ignore').readline())
    print("test1:", open("iwslt2016/segmented/test.de.bpe",
                         'r', encoding='utf-8', errors='ignore').readline())
