...
if __name__ == '__main__':
    ...
    from utils import standardization2
else:
    from nfvmaddpg.model.gan.utils.progress_bar import ProgressBar
    from nfvmaddpg.model.gan.utils.utils import standardization2
...
class SparseToPoDataset():
    ...
    def load_data(self):
        ...
        with open("nfvmaddpg/model/gan/data/netGraph_abilene.pickle", 'rb') as f:
        ...
        self.vocab = [line.split()[0] for line in open(
            'nfvmaddpg/model/gan/data/vnf.vocab', 'r', encoding='utf-8', errors='ignore').read().splitlines()]
        self.usage_vocab = [line.split()[0] for line in open(
            'nfvmaddpg/model/gan/data/usage.vocab', 'r', encoding='utf-8', errors='ignore').read().splitlines()]
        np.random.seed(0)
        for node in list(G.nodes):
            ...
            G.nodes[node]['usagelist'] = [0] + [1] * 11
            # G.nodes[node]['usagelist'] = [0] + [1] * 11 + [0]
            # print(G.nodes[node])
        # i = 0
        for edge in list(G.edges):
            G.edges[edge[0],edge[1]]['edgeDatarate'] = edge_features[edge]
            G.edges[edge[0], edge[1]]['edgeLatency'] = edge_features_latency[edge]
        ...
        self.data = all_connected_subgraphs * 10
    ...
    def generate(self, validation=0.1, test=0.1):
        ...
        # (see self._genX)
        self.data_X = np.stack(self.data_X) # 628 12 12
    ...
    def _generate_encoders_decoders(self):  # 需要改  每个网络节点处理的vnf实例的index
        self.log('Creating vnf-usage encoder and decoder..')
        # vocab = [line.split()[0] for line in open(
        #     'E:/MANFV/NFVGAN/data/vnf.vocab', 'r', encoding='utf-8', errors='ignore').read().splitlines()]
        self.vnf_encoder = {token: idx for idx, token in enumerate(self.usage_vocab)}
        self.vnf_decoder = {idx: token for idx,
                            token in enumerate(self.usage_vocab)}
        ...
        self.vnf_num_types = len(self.usage_vocab)  # 原子种类数目
        self.log('Created vnf encoder and decoder with {} usage types!'.format(
            self.vnf_num_types))
    ...
    def _genA(self, topo, connected=True, max_length=None):
        ...
        for begin, end in zip([edge[0] for edge in list(topo.edges)], [
            ...
            A[begin, end] = topo.edges[begin, end]['edgeDatarate']  # 邻接矩阵值为对应链路的带宽
            A[end, begin] = topo.edges[end, begin]['edgeDatarate']
        # A[np.where(A.any(axis=1))[0], :] = standardization(A[np.where(A.any(axis=1))[0], :])
        ...
        return standardization2(A) if connected and (degree > 0).all() else None

    def _genX(self, topo, max_length=None):

        # max_length = max_length if max_length is not None else mol.GetNumAtoms()
        # list1 = [np.array(topo.nodes[node]['usagelist'], dtype=np.int32)
        #  for node in list(topo.nodes)]
        # list2 = [np.array([0] * 12, dtype=np.int32)
        #  for _ in range(max_length - len(list(topo.nodes)))]
        return np.stack([np.array(topo.nodes[node]['usagelist'], dtype=np.int32) for node in list(topo.nodes)] + [np.array([0] * 12, dtype=np.int32) for _ in range(max_length - len(list(topo.nodes)))])
    ...
    def _genF(self, topo, max_length=None):
        ...
        # G.nodes[node]['nodeCostAct'] = np.random.random()
        # G.nodes[node]['vnf_instances'] = [0] * 17
        features = np.array([[topo.nodes[node]['nodeCap'], topo.nodes[node]['nodeMem'], topo.nodes[node]['nodeRel'], topo.nodes[node]['nodeCostCap'], 
                              topo.nodes[node]['nodeCostMem'], topo.nodes[node]['nodeCostRun'], topo.nodes[
                                  node]['nodeCostAct'], *topo.nodes[node]['vnf_instances']
                              ] for node in list(topo.nodes)], dtype=np.float)
        # features = np.array([[*[topo.nodes[]],  # true false 长度为5
    ...
    def matrices2topo(self, node_labels, edge_labels, strict=False):
        ...
        a_index = 0
        a_row = 0
        for rindex, row in enumerate(node_labels):
            if row[0] >= 0.1:
                continue
        ...
        gnodes = list(G.nodes)
        for start, end in zip(*np.where(edge_labels > 0.5)):
            if start > end and not (start not in gnodes and end not in gnodes) and end in origitopo.adj[start]:
                ...
                G.edges[int(start), int(end)]['edgeDatarate'] = edge_labels[start, end]
    ...
    #         if start > end and not (start not in gnodes and end not in gnodes) and end in origitopo.adj[start]:
    #             if start not in gnodes:
    #                 G.add_nodes_from([(start, {'usagelist': [0] * 13})])
    #             if end not in gnodes:
    #                 G.add_nodes_from([(end, {'usagelist': [0] * 13})])
    #             G.add_edge(int(start), int(end))  # 添加链路
    #             G.edges[int(start), int(end)]['edgeDatarate'] = edge_labels[start, end]
...
if __name__ == '__main__':
    ...
    data.load_data()
    data.generate()
    data.save('nfvmaddpg/model/gan/data/vnftopo.sparsedataset')
