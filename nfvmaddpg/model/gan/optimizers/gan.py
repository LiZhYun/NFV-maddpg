...
class GraphGANOptimizer(object):
    ...
    def __init__(self, model, learning_rate=1e-3, feature_matching=True):
        self.la = tf.placeholder_with_default(1., shape=())  # 调节两个损失之间的折衷

        with tf.name_scope('losses'):
            eps = tf.random_uniform(tf.shape(model.logits_real)[:1], dtype=model.logits_real.dtype) # 表示从真实数据和生成数据中采样的随机值

            x_int0 = model.adjacency_tensor * tf.expand_dims(tf.expand_dims(eps, -1), -1) + model.edges_softmax * (1 - tf.expand_dims(tf.expand_dims(eps, -1), -1))
            ...
            grad0, grad1 = tf.gradients(model.D_x((x_int0, None, x_int1), model.discriminator_units), (x_int0, x_int1)) # 995   95

            self.grad_penalty = tf.reduce_mean(((1 - tf.norm(grad0, axis=-1)) ** 2), -1) + tf.reduce_mean(
                ((1 - tf.norm(grad1, axis=-1)) ** 2), -1, keep_dims=True) # shape 批次样本数,单个值

            self.loss_D = - model.logits_real + model.logits_fake # shape 1
            self.loss_G = - model.logits_fake # shape 1
            self.loss_V = (model.value_logits_real - model.rewardR) ** 2 + (
                    model.value_logits_fake - model.rewardF) ** 2 # shape 1 sigmoid
            self.loss_RL = - model.value_logits_fake # shape 1 sigmoid
            self.loss_F = (tf.reduce_mean(model.features_real, 0) - tf.reduce_mean(model.features_fake, 0)) ** 2 # 64
