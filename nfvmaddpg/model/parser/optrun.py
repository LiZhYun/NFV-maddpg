...
from optimization.parser2 import Parser
...
reqtype = input("Enter request type (e.g., 'req150'): ").rstrip()
# print( reqtype
netname = input("Enter network name (e.g., 'abilene'): ").rstrip()
objective_mode = input("Enter optimization objective (e.g., 'lat' to minimize total latency, 'use' to minimize the number of used network nodes, 'dr' to maximize the total remaining link capacity in network): ").rstrip()
...
for i,req in enumerate(request_list):
    ...
    placementInput_list.append(reqPlacementInputList)   # 多个请求的各种可能的顺序
    
print( "########", placementInput_list)
all_combinations = list(product(*placementInput_list))  # 多个请求之间的组合
...
chosenCombsToPlace = compute_3dpareto_front(list(combinationTotalDataRate.values()), list(combinationTotalComputationalReq.values()), list(combinationNumInstances.values()), maxX = False, maxY = False, maxZ = False)
...
for num,c in enumerate(all_combinations):
    ...
    g = pickle.load(open("netGraph_" + netname + ".pickle","rb"))
