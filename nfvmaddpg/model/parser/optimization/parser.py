...
import sys, os
# sys.path.append('./optimization')  # 为了导入父目录的文件而进行的设定
from nfvmaddpg.model.parser.optimization.placement_input import PlacementInput
...
class Parser:
	def __init__(self, req):	
		...
		self.placementInput = dict()  #作为Transformer的输入
		# A list to store the pairs of VM/VNF instances that should be placed
		# and connected to each other
		self.pairs = []
		# A dictionary to store the modules after parsing the request chain  对应key为chain上module第一个字符的index，值为storeInstance等(平行 分裂等)
		self.parseDict = dict()
...
#		self.instance = Word(alphas + "_" + nums)
		self.instance = Word(alphanums + "_" + alphanums)  # req0_a1
		...
		self.optorder << "(" + self.instance + ZeroOrMore("," + self.instance) + ")"   # (u1,u2)
		...
		# u1[u2|u3.(u4,u5)|u6]
		# split包含：
		# 1.一个VNF，它可以对不同分支上的流进行分类和拆分
		# 2.在流到达分支之前要遍历的可选最佳绑定组合
		# 3.分支可以由单个VNF（或服务端点）组成，也可以由多个VNF组成，或者可以是空分支（pass），用于跳过服务结构的一部分。如果分支相同，则只需指定一次。如果所需复制的数量已知，则可以指定它；如果不知道，则可以将其保留为打开状态，以便在部署时指定。
	...
	def parseParallel(self, s, loc, toks):
		...
		last_mod = ts[delimiters[1] - 1]
		#jump = loc + sum(len(str(x)) for x in ts) + 1
		jump = -1
		jump_lastmod = loc + sum(len(str(t)) for t in ts)
		self.parseDict[loc] = storeParallel(func, num, funcs, mod, jump, jump_lastmod)
		return toks
			...
			if isinstance(s, storeParallel):
				...
				for i,f in enumerate(s.funcs[1:]):
					...
					if part == 1: # 并行函数后的第一个
						...
						for q in range(0, s.num):
							...
							# if self.parseDict[s.funcs[i+1]].func + suffix + "_" + str(q) not in self.U_out:  # undo
					...
					# branches that are copies of the module in the request chain
						...
						# U and r and UF and remove the original one from U
							if self.parseDict[s.funcs[i]].func + suffix not in self.U_out:
								self.U_out.append(self.parseDict[s.funcs[i]].func + suffix)
								self.r_out[self.parseDict[s.funcs[i]].func + suffix] = self.req.r[self.parseDict[s.funcs[i]].func + suffix]
								self.UF_out[self.parseDict[s.funcs[i]].func + suffix] = self.req.UF[self.parseDict[s.funcs[i]].func + suffix]
								self.in_rate[self.parseDict[s.funcs[i]].func + suffix] = 0
								if self.parseDict[s.funcs[i]].func + suffix != self.parseDict[s.funcs[i]].func + suffix and self.parseDict[s.funcs[i]].func + suffix in self.U_out:
							...
							if not insert:
								...
								# Calculate the incoming data rate for every VNF that
								# has all its incoming pairs processed
								# print (self.parseDict[s.funcs[i+1]].func + suffix)
								# print (self.parseDict[s.funcs[i]].func + suffix)
								self.in_rate[self.parseDict[s.funcs[i+1]].func + suffix] += self.r_out[self.parseDict[s.funcs[i]].func + suffix][rindex] * self.in_rate[self.parseDict[s.funcs[i]].func + suffix]
								insert = True
							rindex = 0
				...
				for q in range(0, s.num):
					if part == 2:
						...
						p = self.createPairs(s.mod, s.jump, [self.parseDict[s.funcs[-1]].func + suffix], suffix + "_" + str(q))  # 并行函数在最后
					for r in p:
						prevs.append(r)
	...
	def create_pairs(self):
		...
		self.placementInput['UF'] = self.UF_out #VNF 类型
	...
	def preparse(self):
		...
		for i in range(0, len(self.req.chain)):
			if i in self.parseDict:
				...
				if isinstance(self.parseDict[i], storeParallel):
					self.parallel_num[self.parseDict[i].func] = self.parseDict[i].num
	...
	def undofixforopt(self, jump, begin, end):
		for key, value in self.parseDict.items():
			if isinstance(value, storeOptorder) and value.jump != -1:
				value.jump = -1
				for f in value.funcs:
					self.parseDict[f].jump = -1
			elif isinstance(value, storeParallel) and value.jump != -1:
				for f in value.funcs:
					if self.parseDict[f].func == value.func and len(value.funcs) == 1:
						self.parseDict[f].jump = value.mod
					else:
						self.parseDict[f].jump = -1
...
class storeParallel():
	def __init__(self, func, num, funcs, mod, jump, jump_lastmod):
		self.func = func
		self.num = num
		self.funcs = funcs 
		self.mod = mod 
		self.jump = jump 
		self.jump_lastmod = jump_lastmod 
	def __str__(self):
