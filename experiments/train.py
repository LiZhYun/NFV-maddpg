...
def parse_args():
    ...
    parser.add_argument("--num_agents", type=int, default=4, help="works for agents")
...
def central_agent(net_params_queues, exp_queues, arglist):
    ...
    with U.multi_threaded_session():  # 只使用一个cpu的会话
        ...
        logging.info('# Master Multi-Agents: pid={0} Starting...'.format(os.getpid()))
        # print('Starting iterations...')

        while True:
            # logging.info('# Master Multi-Agents: pid={0} Assembling Experiences from Worker Multi-Agents...'.format(os.getpid()))
            logging.info('# Master Multi-Agents: pid={0} Synchronizing the network parameters to Worker Multi-Agents...'.format(os.getpid()))
            for trainer_index, agent in enumerate(trainers):
                generator_vars, discriminator_vars, gcn_vars, transformer_vars = tf.get_default_session().run([agent.generator_vars, agent.discriminator_vars, agent.gcn_vars, agent.transformer_vars])
                for i in range(arglist.num_agents):
                    # generator_vars, discriminator_vars, gcn_vars, transformer_vars = net_params_queue[i][trainer_index].put()
                    net_params_queues[i][trainer_index].put([generator_vars, discriminator_vars, gcn_vars, transformer_vars])
            # info = psutil.virtual_memory()
            # logging.info('内存使用：%s' % str(psutil.Process(os.getpid()).memory_info().rss))
            # logging.info("# Request Num %s" % str(REQ_NUM))
            ...
            for i, agent in enumerate(trainers):
                rwd_avg = 0.0
                for num in range(arglist.num_agents):
                    obs_tmp, action_tmp, rew_tmp, new_obs_tmp, done_tmp = exp_queues[num][i].get()
                    # action_n[i][0] = action_n[i][0].reshape(action_n[i][0].shape[1], action_n[i][0].shape[2])
                    # action_n[i][1] = action_n[i][1].reshape(action_n[i][1].shape[1], action_n[i][1].shape[2])
                    agent.experience(obs_tmp, action_tmp, rew_tmp, new_obs_tmp, done_tmp, terminal)

                    rwd_avg += rew_tmp
                rwd_avg /= arglist.num_agents
                episode_rewards[-1] += rwd_avg
                agent_rewards[i][-1] += rwd_avg
                # logging.info("Agent_%d's Replay buffer size: %s" % (i,str(agent.replay_buffer.sizeofstorage())))
            # after = psutil.Process(os.getpid()).memory_info().rss
            # logging.info('存入经验后内存变化：%s' % str(after - prev))
            # obs_n = new_obs_n
            ...
            # update all trainers, if not in display or benchmark mode
            logging.info('# Master Multi-Agents: pid={0} Updating...'.format(os.getpid()))
            loss = None
            for agent in trainers:
                agent.preupdate()
            # prev = psutil.Process(os.getpid()).memory_info().rss
...
def agent(net_params_queue, exp_queue, arglist, REQ_NUM, share_lock):
    # global REQ_NUM
    with U.multi_threaded_session():  # 只使用一个cpu的会话
        # Create environment
        env = make_env(arglist.scenario, arglist, arglist.benchmark)
        ...
        features = obs_n[0][1]
        edge_features = obs_n[0][-2]
        logging.info('# Worker Multi-Agents: pid={0} Using policy {1} '.format(os.getpid(), arglist.policy))
        trainers = get_trainers(env, obs_shape_n, arglist, features, edge_features)
        # print('Using policy {} '.format(arglist.policy))
        ...
        # Initialize
        # U.initialize()
        logging.info('# Worker Multi-Agents: pid={0} Initiating synchronization of the network parameters from Master Multi-Agents...'.format(os.getpid()))
        for trainer_index, agent in enumerate(trainers):
            generator_vars, discriminator_vars, gcn_vars, transformer_vars = net_params_queue[trainer_index].get()
            tf.get_default_session().run(agent.set_generator_vars_params_op, feed_dict={
                i: d for i, d in zip(agent.input_generator_vars_params, generator_vars)
            })
            tf.get_default_session().run(agent.set_discriminator_vars_params_op, feed_dict={
                i: d for i, d in zip(agent.input_discriminator_vars_params, discriminator_vars)
            })
            tf.get_default_session().run(agent.set_gcn_vars_params_op, feed_dict={
                i: d for i, d in zip(agent.input_gcn_vars_params, gcn_vars)
            })
            tf.get_default_session().run(agent.set_transformer_vars_params_op, feed_dict={
                i: d for i, d in zip(agent.input_transformer_vars_params, transformer_vars)
            })
        # Load previous results, if necessary
        # if arglist.load_dir == "":
        #     arglist.load_dir = arglist.save_dir
        ...
        while True:
            ...
            #     break
            # logging.info("# Worker Multi-Agents: pid={0} Reward is {1}" .format(os.getpid(), rew_n))
            logging.info('# Worker Multi-Agents: pid={0} Synchronizing the network parameters from Master Multi-Agents...' .format(os.getpid()))
            for trainer_index, agent in enumerate(trainers):
                generator_vars, discriminator_vars, gcn_vars, transformer_vars = net_params_queue[trainer_index].get(
                )
                tf.get_default_session().run(agent.set_generator_vars_params_op, feed_dict={
...
def train(arglist):
    ...
    for i in range(arglist.num_agents):
        ...
        net_params_queues_n.append(net_params_queues)
        exp_queues_n.append(exp_queues)
    
    # create a coordinator and multiple agent processes
    # (note: threading is not desirable due to python GIL)
    coordinator = mp.Process(target=central_agent,
                             args=(net_params_queues_n, exp_queues_n, arglist))
    coordinator.start()

    REQ_NUM = mp.Manager().Value(int, 0)
    share_lock = mp.Manager().Lock()
    agents = []
    for i in range(arglist.num_agents):
        agents.append(mp.Process(target=agent,
                                 args=(
                                       net_params_queues_n[i],
                                       exp_queues_n[i], arglist, REQ_NUM, share_lock)))
    for i in range(arglist.num_agents):
        agents[i].start()
